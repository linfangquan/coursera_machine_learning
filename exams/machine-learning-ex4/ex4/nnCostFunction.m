...
% Part 1
logit1 = [ones(m, 1) X] * Theta1';
logit1 = sigmoid(logit1);
logit2 = [ones(size(logit1, 1), 1) logit1] * Theta2';
p = sigmoid(logit2);
I = eye(max(y));
y_onehot = I(y,:);

J = sum(sum(-y_onehot .* log(p) - (1 - y_onehot) .* log(1 - p))) / m + ...
...
for i = 1 : m
	...
	delta2 = delta2(2 : end);
	delta2 = delta2 .* sigmoidGradient(logit2)';
